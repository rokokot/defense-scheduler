...
DEFAULT_SETTINGS = {
    # General
    "input_data": 'documented_example_trivial',
    ...
    # Showing solutions (Gantt charts)
...
class DefenseRosteringModel(cp.Model):
    def __init__(self, cfg):
        ...
        if cfg['model'] == 'allocation_intvar':
            ...
            if cfg['adjacency_objective']:
                ...
                self.add([self.dummy == 1])
                self.maximize(self.dummy)
        elif cfg['model'] == 'allocation_boolvar1' or cfg['model'] == 'allocation_boolvar2':
            ...
            if cfg['adjacency_objective']:
                if cfg['model'] == "allocation_boolvar1":
                    self.adj_obj, self.adj_obj_ub = self.adjacency_objective_allocation_boolvar()
                if cfg['model'] == "allocation_boolvar2":
                    self.adj_obj, self.adj_obj_ub = self.adjacency_objective_allocation_boolvar2()
                # print("Upper bound: ", self.adj_obj_ub)
                ...
                for d2 in defenses:
                    if d1 != d2:
                        for t in range(1, self.no_timeslots):
                            adjacency_objective += (cp.any(self.planned[d1, :, t - 1]) & cp.all(
                                self.planned[d1, :, t - 1] == self.planned[d2, :, t]))
                        pairs_count += 1
    ...
    def adjacency_objective_allocation_boolvar2(self):
        adjacency_objective = 0
        time_objective_ub = 0
        pairs_count = 0
        df_evaluators = (
        ...
        '''self.time_diff = cp.intvar(lb=0, ub=self.no_timeslots, shape=(self.no_defenses, self.no_defenses), name="time_diff")
        self.same_room = cp.boolvar(shape=(self.no_defenses, self.no_defenses), name="same_room")
        
        for d1 in range(self.no_defenses):
            for d2 in range(self.no_defenses):
                self.add(self.same_room[d1, d2] == cp.any(self.planned[d1, r, :]) & cp.any(self.planned[d2, r, :]))'''

        '''self.timeslot = cp.bool(lb=0, ub=self.no_timeslots, shape=self.no_defenses, name="timeslot")
        for d in range(self.no_defenses):
            for t in range(self.no_timeslots):
                self.add(self.timeslot[d,t] == cp.any(self.planned[d, :, t]))

        self.room = cp.intvar(lb=0, ub=self.max_rooms, shape=self.no_defenses, name="room")
        for d in range(self.no_defenses):
            for r in range(self.max_rooms):
                self.add(self.timeslot[d,r] == cp.any(self.planned[d, r, :]))'''
                    ...
                    if res['in_room'][d] == r:
                        ...
                        if st != None:
                            ...
                            rows.append({
                                ...
                                **({"co_supervisor": info["co_supervisor"]} if not pd.isna(
                                    info['co_supervisor']) else {}),
                                "start": pd.to_datetime(st, unit="h", origin=model.first_day),
                                "end": pd.to_datetime(en, unit="h", origin=model.first_day)
                            })

    unavailable_intervals = get_unavailable_intervals(model, cfg,
                                                      f'input_data/{cfg["input_data"]}/unavailabilities.csv',
                                                      'room')
                ...
                if st is not None:
                    ...
                    for r in range(model.max_rooms):
                        if res['in_room'][d] == r:
                            ...
                            break
                    # room_name = model.df_rav.loc[model.df_rav["room_id"] == f'room-{room_id}', "name"].iloc[0]
                    columns_to_include = ['supervisor', 'co_supervisor', 'assessor1', 'assessor2', 'mentor1', 'mentor2',
                                          'mentor3', 'mentor4']
...
def get_value(var, solver=None):
    return solver.Value(var) if solver is not None else var.value()
...
# convert_result stays the same
def convert_result(model, solver=None):
    """
    ...
    if hasattr(model, "in_room"):
        data["in_room"] = [v.value() for v in model.in_room.flat]

    if hasattr(model, "start_times"):
        data["start_times"] = [v.value() for v in model.start_times.flat]

    if hasattr(model, "is_planned"):
        data["is_planned"] = [bool(v.value()) for v in model.is_planned.flat]
    else:
    ...
    if hasattr(model, "planned"):
        room_and_time = []
        for d in range(model.no_defenses):
            vals = np.array([v.value() for v in model.planned[d, :, :].flat]).reshape(
                model.planned.shape[1:]
            )
            i_arr, j_arr = np.where(vals == 1)
            if len(i_arr) == 0:
                i, j = None, None
            else:
        ...
        data["in_room"] = [i if i is not None else 0 for i, _ in room_and_time]
        data["start_times"] = [j if j is not None else 0 for _, j in room_and_time]
        data["is_planned"] = [i is not None for i, _ in room_and_time]

    return data


'''from pathlib import Path
csv_path = Path("experimental_results/adj_obj_incr.csv")
csv_path.parent.mkdir(parents=True, exist_ok=True)

# Write header once if file does not exist


def log_solution(solver, model, solution_count, solution_time, adjacency_objective, defenses_planned):
...
if __name__ == "__main__":
    ...
    log_solution(cfg['solver'], cfg['model'],0,0,0,0)'''
    ...
    def callback_func():  # no arguments!
        ...
        '''log_solution(
            solver=cfg['solver'],
            model=cfg['model'],
            solution_count=callback.solution_counter,
            solution_time=callback.sol_time - callback.start_time,
            adjacency_objective=adj_obj_value,
            defenses_planned=defenses_obj_value
        )
